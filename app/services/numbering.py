...
_MARKER_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    ("ㆍ", re.compile(r"^[ㆍ·•]")),
    ("※", re.compile(r"^※")),                        # 비고·출처 표기
    ("*", re.compile(r"^\*(?=\s|\[)")),               # 각주·출처 (예: "* [출처] …"), **굵게** 제외
]
...
# 문장 중간 분할용 마커: 공백·문장 종결 부호 뒤에 오는 기호류·괄호류·"N)" 형태만.
# 순수 숫자("1.")·한글("가.") 마커는 날짜("2026. 7. 13.")·약어와 충돌하므로
# 중간 분할에서는 제외한다 — 줄 시작 판정(starts_with_marker)은 전체를 본다.
# 경계에 공백 외 종결 부호(. ) ] ! ?)를 허용하는 이유: T3Q 응답이
# "…있다.○ 다음 항목"처럼 구분자 없이 이어지는 경우가 있다 (2026-07-21).
# ㅇ(이응)·*는 뒤 공백을 요구해 일반 단어·**굵게**와의 오탐을 막는다.
