...
## C. 구조 인식 전량 재작성 채움 (fill 재설계)

**배경** (오류 캡처 실측): 스텁 단어 템플릿(`title`·`표제목`·`표내용`,
노드 31개)은 regex 표식도 가이드 스타일도 없어 종전 fill이 `[출처]`
1개 노드만 채웠고, 나머지 템플릿 원문이 그대로 남아 "엉뚱한 내용"으로
보였다. 사용자 결정: **fill을 구조 인식 재작성으로 대체** — 어떤
템플릿이든(과거에 채워진 문서 포함) 구조(제목 자리·헤딩 번호/개요
기호·표 형태·라벨)만 남기고 사용자 내용으로 전량 다시 채운다.

**설계**:
- fill 대상 = **문서의 모든 노드** (빈 셀 포함 — A1이 물리적 쓰기 보장).
  기존 텍스트는 프롬프트에 구조 힌트로 제공(B2 그리드)하고, LLM이
  제목/헤딩/머리글/값 칸에 새 내용을 배치하며 무관한 옛 본문은
  `new_text: ""`로 지운다. 라벨·머리글 유지/대체는 LLM 판단(사용자 결정).
- **대형 문서 안전판**: `FILL_NODE_LIMIT`(기본 300) 초과 시 표식·가이드
  중심(B3 방식)으로 축소 + notes 안내. 표식조차 없으면 앞쪽 상한 개수만.
  (서식1급 5,386노드 전량 순회 시 호출 수백 회 폭주 방지)
- 의도 분류·병합 프롬프트의 fill 정의에 "재작성" 포함,
  휴리스틱 키워드에 "다시 작성"·"재작성" 추가.
- **한계**: hwpx 자동번호(paraPr numbering)는 텍스트로 노출되지 않아
  LLM이 번호 값을 인지하지 못한다 — 다만 텍스트 교체 방식이라 번호
  모양·서식 자체는 항상 보존된다.
