...
### 4.1 현재 상태 — 실양식 확보·재실측 완료

Phase 4 시점에는 실양식이 없어 python-hwpx 동봉 샘플과 builder 생성 문서로
**대체 실측**(§4.2)을 수행했다. 이후 **실양식(`실양식.hwpx`)을 입수해 §4.3 체크리스트로
재실측을 완료했다** (2026-07-11, 결과는 §4.4).
...
### 4.4 실양식 재실측 결과 (2026-07-11)

대상: 저장소 루트의 `실양식.hwpx` (74,084 bytes). 원본·산출물은 git 미커밋
(루트 원본은 `.gitignore` 등재, 산출물은 `data/real_form_check/` — data/는 gitignore).
측정 스크립트 출력 전문: `data/real_form_check/재실측_결과.txt`.

| §4.3 단계 | 결과 |
|---|---|
| 1. extract | ✅ 엔트리 11개(mimetype, version.xml, header/section0/content.hpf, Preview 2종, settings.xml, META-INF 3종), 섹션 1개 |
| 2. validate(원본) | ✅ ok=True, errors 없음 |
| 3. parse | ✅ 노드 194개 = body_text 1(빈 문단) + table_cell 193(표 1개). **병합 셀 185개(span>1)** — 표 중심 양식. 중첩 표 없음 |
| 4. to_html 불변식 | ✅ `data-id` 194개 == 전역 id 집합 (page_count 1). 대규모 병합 셀 구조에서 불변식 성립 — §4.2의 미측정 항목 중 "셀 병합" 해소 |
| 5. placeholder | ✅ bracket 유형 11건 탐지(`[  ]`, `[√]` 등 체크박스성 표식). angle/date_stub/filler 없음. 체크박스 `[√]`는 채움 대상이 아니라 선택 표식이므로 fill 파이프라인 사용 시 주의 필요 |
| 6. apply_edits 왕복 | ✅ 셀 1개 편집(id=0) → applied=[0], 편집본 validate 통과, 재파싱 반영 확인, **비편집 193개 노드 원문 불변**. ※ 문단 편집은 비어 있지 않은 body_text가 없어 셀로만 수행(문서 특성) |
| 7. 한컴 열람 | ⏳ 원본·편집본(`data/real_form_check/실양식_편집본.hwpx`)을 한컴오피스 2022로 열어 둠 — §3 체크리스트 육안 확인은 검수자 수행 대기 |
| 8. 회귀 테스트 편입 | 실양식은 내부 양식으로 재배포 가능 여부 미확인 → 픽스처 추가 보류, 측정 수치만 본 문서에 기록 |

관찰 사항:
- 다중 섹션 실측은 여전히 미수행 (이 양식은 단일 섹션). §4.2의 한계 중 "수백 노드·셀 병합"은 해소, "다중 섹션·중첩 표"는 합성 픽스처 테스트로만 검증된 상태 유지.
- placeholder 패턴이 실양식의 체크박스(`[√]`)를 bracket으로 잡는다 — 채움 파이프라인에서 체크박스를 텍스트로 덮어쓰지 않도록 향후 패턴 정교화 후보 (`docs/limitations.md` 향후 과제 연계).

### 4.5 재난계획서 문서 템플릿 3종 재실측 결과 (2026-07-11)

대상: 루트의 `(양식) 문서 템플릿_01~03.hwpx` (원본은 `.gitignore` 등재, 산출물은
`data/real_form_check/`). 측정 출력 전문: `data/real_form_check/재실측_결과_템플릿3종.txt`.
§4.3의 1~6단계를 각각 수행했고 **3종 전부 전 단계 통과**.

| 항목 | 템플릿_01 | 템플릿_02 | 템플릿_03 |
|---|---|---|---|
| 크기 / 엔트리 / 섹션 | 31KB / 11 / 1 | 33KB / 11 / 1 | 500KB / 14 / 1 |
| BinData(이미지) | 0 | 0 | **3** |
| 노드 (body/cell, 표 수) | 70 (28/42, 4개) | 79 (31/48, 6개) | 97 (19/78, 6개) |
| 병합 셀 / 중첩 표 | 0 / 0 | 0 / 0 | 0 / 0 |
| validate(원본) | ✅ | ✅ | ✅ |
| to_html data-id 일치 | ✅ 70 | ✅ 79 | ✅ 97 |
| placeholder | bracket 10건 (`[문서 주제]` 등) | bracket 11건 | 0건 |
| apply_edits 왕복 (문단+셀) | ✅ 비편집 68개 불변 | ✅ 77개 불변 | ✅ 95개 불변 |

관찰 사항:
- 템플릿_01·02의 `[문서 주제]`류 bracket placeholder는 **시나리오 A(양식 채움)의 실제 채움
  대상으로 적합** — fill 파이프라인 실데모에 이 두 템플릿을 권장.
- 템플릿_03은 placeholder가 없고 이미지 3개 포함 — 시나리오 B(기존 문서 수정)·이미지 렌더링
  확인용으로 적합. `hwpx_to_html`의 BinData→base64 인라인 경로가 실물에서 처음 검증됨.
- 실양식(§4.4)과 달리 3종 모두 일반 문단이 있어 문단·셀 편집을 모두 왕복 검증함.
- 한컴 열람(§4.3 7단계) 육안 확인은 검수자 수행 대기.
