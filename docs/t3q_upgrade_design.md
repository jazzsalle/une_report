...
## 부록 A. 표준 템플릿 규약 (2026-07-15)

`templates/*.hwpx`는 생성 보고서에 서식을 입히는 **표본 문서**다
(`app/services/report_template.py`가 인식·조립). 새 템플릿은 아래 순서를 지킬 것:

1. p0 = 문서 제목 표본 (예: "코로나19 대비 계획서")
2. p1 = 부제 표본 ("서면 보고 / 날짜 / 작성자") — 내보내기 시 기준정보로 자동 구성
3. 이후 처음 나오는 문단이 각 표본이 된다:
   `1.`로 시작 → 헤딩1 / `가.` → 헤딩2 / `○` → 개조식1 / `-` → 개조식2
   (개조식 들여쓰기는 텍스트 앞 공백으로 표현 — 그대로 보존됨)
4. hp:tbl 하나(빈 표) = 표 서식 표본 (선택 — 없으면 표는 텍스트 행으로 폴백)

내보내기 규칙: 표 열폭 160mm 균등, 새 개요기호마다 줄바꿈, 편집 문단 lineseg 제거.
docx는 표본 charPr(크기·굵기)·paraPr(가운데)·들여쓰기를 근사 적용.
